  ...
  return (
    <div className="min-h-screen bg-secondary py-12 px-4">
      <div className="max-w-xl mx-auto text-center">
        <h1 className="font-display font-bold text-3xl md:text-4xl uppercase tracking-tight">Votre résumé Wrysst</h1>
      ...
      <div className="max-w-xl mx-auto bg-white rounded-lg shadow-sm border border-border mt-8 p-8">
        <div className="flex items-center gap-4 pb-6 border-b border-border">
          ...
          <div>
            <p className="text-[10px] tracking-widest font-bold text-muted-foreground">{watch.brand}</p>
        ...
        <div className="bg-secondary rounded-lg p-4 mt-6 text-xs text-muted-foreground">
          Votre accès Wrysst inclut l'assurance complète vol et casse, l'entretien par un maître horloger agréé, le polissage avant chaque livraison, et la flexibilité totale de changer de montre à la demande.
        </div>

        <button
          onClick={() => navigate({ to: "/confirmation" })}
          className="w-full mt-6 bg-primary text-primary-foreground py-4 rounded-md font-bold tracking-wider text-sm hover:bg-primary/90"
        >CONFIRMER MA RÉSERVATION →</button>
        <Link to="/watch/$id" params={{ id: watch.id }} className="block text-center mt-4 text-xs text-muted-foreground hover:underline">← Modifier ma sélection</Link>
      </div>
    </div>
  );
}
...
function Row({ k, v, bold, highlight }: { k: string; v: string; bold?: boolean; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-border last:border-0">
      ...
      <span className={`text-sm ${bold ? "font-bold text-base" : ""} ${highlight ? "bg-black-edition text-black-edition-foreground px-2 py-0.5 rounded text-[10px] font-bold tracking-widest" : ""}`}>{v}</span>
    </div>
  );
}
